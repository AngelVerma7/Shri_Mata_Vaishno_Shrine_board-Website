...
// Submit yatra booking (mock handler)
app.post('/api/yatra/book', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: 'Booking submitted successfully (mock response)',
      referenceId,
      booking: { ...bookingData, referenceId, status: 'pending' }
    });
  } catch (error) {
    console.error('Booking error:', error);
    res.status(500).json({
...
// Get booking by reference ID (mock)
app.get('/api/yatra/booking/:referenceId', (req, res) => {
  const { referenceId } = req.params;

  // In a real app, this would look up the DB. For now, just simulate:
  res.json({
    success: true,
    booking: {
      referenceId,
